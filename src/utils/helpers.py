...
def get_overdue_tasks(tasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for task in tasks:
        ...
        if due_date:
            try:
                task_date = parse_date_string(due_date)
                if task_date and task_date.date() < today and task.get('status') != 2:  # Not completed
                    results.append(task)
            except Exception as e:
                # Log parsing errors for debugging
                continue
...
def create_task_builder(title: str, content: str = "", project_id: str = None,
                       priority: int = 0, due_date: str = None) -> Dict[str, Any]:
    """
    Create task builder
